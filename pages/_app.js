...
// import { useSession } from "next-auth/react"; // УДАЛЕНО: больше не используется напрямую здесь
// import { auth } from '../lib/firebase'; // УДАЛЕНО: больше не используется напрямую здесь
// import { signInWithCustomToken, signOut } from 'firebase/auth'; // УДАЛЕНО: больше не используется напрямую здесь

// Импорты CSS (остаются без изменений)
...
// УДАЛЕНО: Компонент FirebaseAuthCustomTokenSync больше не нужен,
// так как Firebase Auth полностью удален.
...
function MyApp({ Component, pageProps: { session, ...pageProps } }) {
    ...
    return (
        <SessionProvider session={session}>
            ...
            {/* УДАЛЕНО: Компонент FirebaseAuthCustomTokenSync, он больше не нужен. */}
